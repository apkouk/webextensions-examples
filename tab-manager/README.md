...
* if the user reloads the tab, or switches tabs, while the popup is open, then the popup won't be able to add-category the page any more (because the content script was injected into the original tab).
...
* it's not possible to inject content scripts into certain pages, including privileged browser pages like "about:debugging" and the [addons.mozilla.org](https://addons.mozilla.org/) website. If the user clicks the add-category icon when such a page is loaded into the active tab, the popup displays an error message.
